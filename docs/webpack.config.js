    ...
    rules: [
      ...
      },
      {
        ...
        exclude: [
          ...
          /switchdin-superset-plugins.*\/node_modules\//,
          /\.test.jsx?$/,
        ],
        include: [
          new RegExp(`${APP_DIR}/src`),
          /superset-ui.*\/src/,
          /switchdin-superset-plugins.*\/src/,
      ...
      {
        test: /\.css$/,
        include: [APP_DIR, /superset-ui.+\/src/, /switchdin-superset-plugins.+\/src/],
        use: [
...
if (isDevMode) {
  ...
  Object.entries(packageConfig.dependencies).forEach(([pkg, version]) => {
    ...
    if (
      (/superset-ui/.test(pkg) || /switchdin-superset-plugins/.test(pkg)) &&
